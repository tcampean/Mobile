...
class MainViewModelWithDao: ViewModel() {
    ...
    private val _successfulOnline = MutableLiveData<Long>()
    val successfulOnline: LiveData<Long>
        get() = _successfulOnline
    ...
    fun insert(product: Product, isOnline: Boolean) = viewModelScope.launch(Dispatchers.IO) {
        _action.postValue(AppConstants.ACTION_CREATE)
        Log.d("YES", "ENTERED IN INSERT")
        if (isOnline) {
                ProductAPI.retrofitService.addProduct(ProductEntity(product.name, product.type, product.description, product.quantity, product.price)).enqueue(object :
                    Callback<ProductEntity?> {
                    override fun onResponse(call: Call<ProductEntity?>, response: Response<ProductEntity?>) {
                        Log.d("Server", "Added product to server Success: " + response.body().toString())
                        viewModelScope.launch(Dispatchers.IO) {
                            try {
                                _successful.postValue(
                                    repository.insert(
                                        response.body()!!
                                        )
                                    )
                        ...
                        _successfulOnline.postValue(1)
                    }

                    override fun onFailure(call: Call<ProductEntity?>, t: Throwable) {
                        Log.d("Server", "Added product to server Failed: " + t.message)
                        viewModelScope.launch(Dispatchers.IO) {
                            try {
    ...
    fun delete(productEntity: ProductEntity, isOnline: Boolean) = viewModelScope.launch(Dispatchers.IO) {
        getProductPosition(productEntity)
        _action.postValue(AppConstants.ACTION_DELETE)
        if (isOnline) {
            ProductAPI.retrofitService.deleteProduct(productEntity.id).enqueue(object :
                Callback<ProductEntity?> {
                override fun onResponse(call: Call<ProductEntity?>, response: Response<ProductEntity?>) {
                    Log.d("Server", "Delete product to server Success: " + response.body().toString())
                    _successfulOnline.postValue(1)
                }

                override fun onFailure(call: Call<ProductEntity?>, t: Throwable) {
                    Log.d("Server", "Delete product to server Failed: " + t.message)
                    _successfulOnline.postValue(-1)
                }
            })
        }
    ...
    fun update(productEntity: ProductEntity, isOnline: Boolean) = viewModelScope.launch(Dispatchers.IO) {
        getProductPosition(productEntity)
        _action.postValue(AppConstants.ACTION_UPDATE)
        if (isOnline) {
            ProductAPI.retrofitService.updateProduct(productEntity.id, productEntity).enqueue(object :
                Callback<ProductEntity?> {
                override fun onResponse(call: Call<ProductEntity?>, response: Response<ProductEntity?>) {
                    Log.d("Server", "Update product to server Success: " + response.body().toString())
                    _successfulOnline.postValue(1)
                }

                override fun onFailure(call: Call<ProductEntity?>, t: Throwable) {
                    Log.d("Server", "Update product to server Failed: " + t.message)
                    _successfulOnline.postValue(-1)
                }
            })
        }
